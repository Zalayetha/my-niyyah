...
export const MenuCard = (props: MenuCardProps) => {
  return (
    <div
      className={twMerge(
        `rounded-3xl p-4 flex flex-col ${props.backgroundColor ?? "bg-slate-900"} relative overflow-hidden`,
        props.className,
      )}
    >
      <Link to={props.link ?? ""} className={props.link && "cursor-pointer"}>
        <div className="flex flex-row justify-between items-center">
          <div
            className={`${props.backgroundColor ? "text-primary" : "text-white"} font-medium text-sm`}
          >
            {props.title}
          </div>
          <Icon icon={props.icon ?? ""} className="text-secondary" />
        </div>
        <div className="mt-6">
          {props.backgroundIcon && (
            <div>
              <Icon
                ...
                className="text-primary/10 me-4 absolute -bottom-8 -right-8"
                fontSize={150}
              />
            </div>
          )}
